...
const CursorGrid = ({
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      if (p.gridOpacity! > 0) {
        ...
        for (let cCol = 0; cCol <= cols; cCol++) {
          const sx = Math.round(offX + cCol * p.cellSize!) + 0.5;
          ctx.moveTo(sx, 0);
          ctx.lineTo(sx, h);
        }
        for (let cRow = 0; cRow <= rows; cRow++) {
          const sy = Math.round(offY + cRow * p.cellSize!) + 0.5;
          ctx.moveTo(0, sy);
          ctx.lineTo(w, sy);
        }
        ctx.stroke();
      ...
      for (let pi = pulses.length - 1; pi >= 0; pi--) {
        ...
        for (let cRow = minRow; cRow <= maxRow; cRow++) {
          for (let cCol = minCol; cCol <= maxCol; cCol++) {
            ...
            if (Math.abs(dist - ringR) < band / 2 && p.maxOpacity! > alphas[i]) { alphas[i] = p.maxOpacity!; touched[i] = now; }
          }
        }
      }
      ...
      for (let i = 0; i < alphas.length; i++) {
        ...
        const [cx, cy] = cellCenter(i);
        ctx.beginPath();
        const sx = cx - half + 0.5;
        const sy = cy - half + 0.5;
        const ss = p.cellSize! - 1;
        if ((ctx as any).roundRect && p.cellRadius! > 0) {
          (ctx as any).roundRect(sx, sy, ss, ss, p.cellRadius);
        } else {
      ...
      if (anyVisible) { raf = requestAnimationFrame(draw); } else { running = false; if (p.gridOpacity! <= 0) ctx.clearRect(0, 0, w, h); }
    };
    ...
    const onPointerMove = (e: PointerEvent) => { const [x, y] = toLocal(e); energize(x, y); wake(); };
