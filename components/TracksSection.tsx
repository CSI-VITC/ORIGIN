...
const TRACKS_DATA = [
  { code: 'TRACK-01', category: 'FINTECH', name: 'FINTECH', desc: 'Architect next-generation decentralized payments, automated clearinghouses, cross-chain liquidity rails, and algorithmic financial instruments.', asciiVariant: 'globe' as const },
  { code: 'TRACK-02', category: 'OPEN INNOVATION', name: 'OPEN INNOVATION', desc: 'Solve unconstrained challenges across any domain. Build innovative SaaS platforms, secure systems, or revolutionary AI architectures.', asciiVariant: 'cube' as const },
];
          ...
          {TRACKS_DATA.map((track, idx) => (
            <div key={track.code} ref={el => { cardRefs.current[idx] = el; }} onMouseEnter={() => setHoveredIdx(idx)} onMouseLeave={() => setHoveredIdx(null)} onClick={() => onSelectTrack(track.name)} className="group cursor-pointer flex flex-col">
              <div className="relative w-full h-[220px] sm:h-[300px] md:h-[340px] bg-[#0A0A0A] border border-[#2A2A2A] group-hover:border-[#FF4D1C]/60 transition-colors overflow-hidden flex items-center justify-center">
                ...
                <AsciiBackground variant={track.asciiVariant} opacity={hoveredIdx === idx ? 0.35 : 0.18} />
                <div className="absolute bottom-3 right-3 md:bottom-4 md:right-4 z-20 font-mono-custom text-[9px] md:text-[10px] text-[#8A8A8A] group-hover:text-[#F2F0EB] bg-[#070707]/80 px-2.5 py-0.5 md:px-3 md:py-1 border border-[#2A2A2A] group-hover:border-[#FF4D1C] transition-all flex items-center gap-1.5 opacity-0 group-hover:opacity-100 translate-y-1 group-hover:translate-y-0"><span>VIEW DETAILS</span><span>→</span></div>
              </div>
